...
class Arguments():
    def __init__(self):
        ...
        self.learning_rate = 1.0  # Learning rate for the optimizer
        ...
        self.no_workers = 50  # Number of federated workers (simulated clients)
...
# Define the Matrix Factorization (GMF) model
class GMF(torch.nn.Module):
    def __init__(self, num_users=num_users, num_movies=num_movies, embedding_size=args.embed_size):
        ...
        self.item_factors = torch.nn.Embedding(num_movies, embedding_size)

    def forward(self, train_data):
        user=train_data[:,0]
        item=train_data[:,1]
        logits = (self.user_factors(user) * self.item_factors(item)).sum(1, keepdim=True)
...
writer.export_scalars_to_json(f"results/fed_mf_{args.no_workers}_{args.epochs}_{args.batch_size}_{args.learning_rate}_{args.federate_after_n_batches}_{args.weight_decay}_{args.top_k}.json")
writer.close()